...
const patch = snabbdom.init([ // Init patch function with chosen modules
  ...
  require('snabbdom/modules/style').default, // for setting properties on DOM elements
]);
	...
	(cell) => h('span.cell',
		{
			...
			style: { color: cell.ch ? colours.getCSSColour(cell.attr) : '' }
		},
		cell.ch);
...
class Term {
	...
	setCursor(x, y) {
		if (this.cursor) {
			this.term[this.cursor.y][this.cursor.x].cursor = false;
			delete this.cursor;
		}
